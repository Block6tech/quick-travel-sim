...
const PlanCard = ({ plan, delay = 0 }: PlanCardProps) => {
  ...
  return (
    <button
      ...
      className="group relative flex items-center justify-between p-4 bg-card rounded-lg shadow-card hover:shadow-card-hover transition-all duration-200 btn-press text-left w-full touch-target"
      ...
      <div className="flex items-center gap-4">
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {plan.validity} · <Wifi className="w-3 h-3 inline" /> {plan.hotspot ? "Hotspot" : "No hotspot"}
          </p>
        </div>
      </div>
